  ...
  static ChangeNotifierProvider<ManageProductionFishHusbandryVM> create({
    ...
    Production? production,
      ...
      ChangeNotifierProvider(
        lazy: false,
        create: (context) =>
            ManageProductionFishHusbandryVM(farmingId: farmingId)..init(),
...
class _ManageProductionPageState extends State<ManageProductionPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<ManageProductionFishHusbandryVM>(
          ...
          return Form(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Finca: ${vm.fishHusbandry?.farmName ?? ''}',
                  ...
                  if (isEditMode)
                    Column(
                      ...
                      children: [
                        Text(
                          'Inversión Inicial: \$${vm.fishHusbandry?.value}',
                          style: Theme.of(context).textTheme.titleMedium,
                        ),
                        Text(
                          'Total Costos y Gastos del cultivo: \$${vm.fishHusbandry?.calculateTotalCostAndExpense().toString().formatNumberToColombianPesos() ?? ''}',
                          style: Theme.of(context).textTheme.titleMedium,
                        ),
                        const SizedBox(
                          height: 12,
                        ...
                        Text(
                          'Ganancia del cultivo: \$${vm.fishHusbandry?.production?.totalValue?.formatNumberToColombianPesos() ?? ''}',
                          style:
  ...
  bool validateIfEmpty(BuildContext context) {
    final vm = Provider.of<ManageProductionFishHusbandryVM>(
      context,
      listen: true,
    );
  ...
  Future<void> createProduction() async {
    final manageVM = Provider.of<ManageProductionFishHusbandryVM>(
      context,
      listen: false,
    );
    ...
    final production = Production(
      createDate: date,
      farmName: manageVM.fishHusbandry?.farmName,
  ...
  Color _getEarningsColors(ManageProductionFishHusbandryVM vm) {
    final value = int.parse(
        vm.fishHusbandry?.production?.totalValue?.replaceAll(',', '') ?? '0');
