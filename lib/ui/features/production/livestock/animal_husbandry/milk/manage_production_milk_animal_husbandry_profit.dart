...
import 'package:amca/ui/features/production/livestock/animal_husbandry/milk/manage_production_milk_animal_husbandry_vm.dart';
...
class ManageProductionPageProfit extends StatefulWidget {
  static ChangeNotifierProvider<ManageProductionMilkAnimalHusbandryVM> create({
    Key? key,
    ...
    Production? production,
      ...
      ChangeNotifierProvider(
        lazy: false,
        create: (context) => ManageProductionMilkAnimalHusbandryVM(farmingId: farmingId)..init(),
...
class _ManageProductionPageState extends State<ManageProductionPageProfit> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<ManageProductionMilkAnimalHusbandryVM>(
          ...
          return Form(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Finca: ${vm.animalHusbandry?.farmName ?? ''}',
                  ...
                  // if (vm.animalHusbandry?.production != null) ...[
                  //   Column(
                  //     crossAxisAlignment: CrossAxisAlignment.stretch,
                  //     children: [
                  //       Text(
                  //         'Inversion Inicial: \$${vm.animalHusbandry?.value ?? ''}',
                  //         style: Theme.of(context).textTheme.titleMedium,
                  //       ),
                  //       Text(
                  //         'Total Costos y Gastos del cultivo: \$${vm.animalHusbandry?.calculateTotalCostAndExpense().toString().formatNumberToColombianPesos() ?? ''}',
                  //         style: Theme.of(context).textTheme.titleMedium,
                  //       ),
                  //       const SizedBox(
                  //         height: 12,
                  ...
                  //       Text(
                  //         //TODO - Cambiar
                  //         'Ganancia del cultivo: \$${vm.animalHusbandry?.totalProfit.formatNumberToColombianPesos() ?? ''}',
                  //         style:
                  ...
                  if (vm.animalHusbandry?.production != null) ...[
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${int.parse(vm.animalHusbandry!.totalProfit) > 0 ? "Ganancia del Cultivo" : int.parse(vm.animalHusbandry!.totalProfit) == 0 ? "Punto Muerto Económico" : "Perdida del Cultivo"}: \$${vm.animalHusbandry?.totalProfit.formatNumberToColombianPesos() ?? ''}',
                          style:
                              Theme.of(context).textTheme.titleMedium?.copyWith(
                                    color: _getEarningsColors(vm),
                                  ),
                        ...
                        if (vm.animalHusbandry?.production != null)
                          for (var item in vm.animalHusbandry!.production!
                              .toList()
                              .reversed) ...[
                            Container(
                              padding:
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          '${item.createDate.month}/${item.createDate.day}/${item.createDate.year}',
                                  ...
                                  //if (vm.animalHusbandry!.production!.length > 1)
                                  IconButton(
                                    icon: Icon(Icons.delete, color: Colors.red),
                                    onPressed: () {
                                      // Obtener el índice del elemento actual en la lista
                                      int index = vm
                                              .animalHusbandry?.production
                                              ?.indexOf(item) ??
                                          -1;
  ...
  bool validateIfEmpty(BuildContext context) {
    final vm = Provider.of<ManageProductionMilkAnimalHusbandryVM>(
      context,
      listen: true,
    );
  ...
  Future<void> createProduction() async {
    final manageVM = Provider.of<ManageProductionMilkAnimalHusbandryVM>(
      context,
      listen: false,
    );
    ...
    final production = Production(
      createDate: date,
      farmName: manageVM.animalHusbandry?.farmName,
  ...
  Color _getEarningsColors(ManageProductionMilkAnimalHusbandryVM vm) {
    final value = int.parse(//TODO - cambiar pos
        vm.animalHusbandry?.totalProfit.replaceAll(',', '') ?? '0');
    return value < 0
        ? Colors.red
        : value == 0
